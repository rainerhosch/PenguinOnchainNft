...
async function main() {
    console.log("Upgrading strategy, adding to BuyList, and distributing yield...");
    ...
    for (const rwa of knownRWAs) {
        ...
        try {
            ...
            // Add to Buy List
            console.log("Adding to Buy List...");
            await (await proxyContract.hotfixAddToBuyList(tokenAddress)).wait();
            ...
            const txApprove = await token.approve(proxyAddress, amountToDistribute);
            await txApprove.wait();

            console.log(`Distributing 1000 ${rwa.symbol} to strategy...`);
            const txDistribute = await proxyContract.distributeYield(tokenAddress, amountToDistribute);
